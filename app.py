...
def doCalcUpdate(calcData, form_data):
    ...
    try:
        ...
        pass
    try:
        pushParam(calcData.diode, "diode_view_from", lambda: int(form_data.get("DiodeViewFrom")))
        pushParam(calcData.diode, "diode_view_to", lambda: int(form_data.get("DiodeViewTo")))
        pushParam(calcData.diode, "calculation_rounds", lambda: int(form_data.get("DiodeRounds")))
        pushParam(calcData.diode, "diode_pulse_width", lambda: float(form_data.get("DiodePulseWidth")))
        pushParam(calcData.diode, "diode_alpha", lambda: float(form_data.get("DiodeAlpha")))
        pushParam(calcData.diode, "diode_gamma0", lambda: float(form_data.get("DiodeGamma0")))
        pushParam(calcData.diode, "diode_saturation", lambda: float(form_data.get("DiodeSaturation")))
        pushParam(calcData.diode, "absorber_half_time", lambda: float(form_data.get("AbsorberHalfTime")))
        pushParam(calcData.diode, "gain_half_time", lambda: float(form_data.get("GainHalfTime")))
        pushParam(calcData.diode, "diode_absorber_shift", lambda: float(form_data.get("DiodeAbsorberShift")))
        pushParam(calcData.diode, "diode_gain_shift", lambda: float(form_data.get("DiodeGainShift")))
        pushParam(calcData.diode, "diode_output_coupler_shift", lambda: float(form_data.get("DiodeOutputCouplerShift")))

        pushParam(calcData.diode, "gain_width", lambda: float(form_data.get("gain_width")))
        pushParam(calcData.diode, "Ta", lambda: float(form_data.get("Ta")))
        pushParam(calcData.diode, "Tb", lambda: float(form_data.get("Tb")))
        pushParam(calcData.diode, "Pa", lambda: float(form_data.get("Pa")))
        pushParam(calcData.diode, "Pb", lambda: float(form_data.get("Pb")))
        pushParam(calcData.diode, "Ga", lambda: float(form_data.get("Ga")))
        pushParam(calcData.diode, "Gb", lambda: float(form_data.get("Gb")))
        pushParam(calcData.diode, "N0a", lambda: float(form_data.get("N0a")))
        pushParam(calcData.diode, "N0b", lambda: float(form_data.get("N0b")))
        pushParam(calcData.diode, "dt", lambda: float(form_data.get("dt")))
        pushParam(calcData.diode, "volume", lambda: float(form_data.get("volume")))
        pushParam(calcData.diode, "initial_photons", lambda: float(form_data.get("initial_photons")))
        pushParam(calcData.diode, "cavity_loss", lambda: float(form_data.get("cavity_loss")))
        pushParam(calcData.diode, "h", lambda: float(form_data.get("h")))
        pushParam(calcData.diode, "diode_update_pulse", lambda: form_data.get("CalcDiodeUpdatePulse"))
        pushParam(calcData.diode, "diode_intensity", lambda: form_data.get("CalcDiodeSelectIntensity"))
        pushParam(calcData.diode, "diode_cavity_type", lambda: form_data.get("CalcDiodeCavityType"))
        pushParam(calcData.diode, "diode_mode", lambda: form_data.get("CalcDiodeSelectMode"))
        pushParam(calcData.diode, "diode_sampling", lambda: form_data.get("DiodeSelectSampling"))
        pushParam(calcData.diode, "diode_sampling_x", lambda: form_data.get("DiodeSelectSamplingX"))
        pushParam(calcData.diode, "start_gain", lambda: float(form_data.get("start_gain")))
        pushParam(calcData.diode, "start_absorber", lambda: float(form_data.get("start_absorber")))
        pushParam(calcData.diode, "rand_factor_seed", lambda: float(form_data.get("rand_factor_seed")))
        pushParam(calcData.diode, "kappa", lambda: float(form_data.get("kappa")))
        pushParam(calcData.diode, "C_loss", lambda: float(form_data.get("C_loss")))
        pushParam(calcData.diode, "C_gain", lambda: float(form_data.get("C_gain")))
        pushParam(calcData.diode, "coupling_out_loss", lambda: float(form_data.get("coupling_out_loss")))
        pushParam(calcData.diode, "coupling_out_gain", lambda: float(form_data.get("coupling_out_gain")))
